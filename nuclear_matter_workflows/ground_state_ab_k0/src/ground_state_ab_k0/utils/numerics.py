"""Small numerical routines used throughout the ground-state workflow."""
...
def simpson_integral(func: Callable[[float], float], a: float, b: float, n: int) -> float:
    """Compute a 1D integral using Simpson's rule."""
    if n % 2 == 1:
        n += 1
...
def bisection_root(
    ...
    """Find a root inside `[left, right]` with the bisection method."""
    f_left = func(left)
    f_right = func(right)
...
def numerical_derivative(func: Callable[[float], float], x: float, h: float = 1.0e-6) -> float:
